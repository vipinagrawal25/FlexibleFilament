import numpy as np
# def write_param(fname='../para_file.in',N=5120,coef_bending=2.5,Y=25,
...
def write_param(fname="../para_file.in",paramdict=None):
	...
	with open(fname, "w") as file:
		...
		file.write("N\tcoef_bending\tY\tcoef_vol_expansion\n")
		file.write("%d %3.2f %3.2f %3.2f\n" %(paramdict['N'],paramdict['coef_bending'],
					paramdict['Y'],paramdict['coef_vol_expansion']))
		file.write("radius\tpos_bot_wall\tsigma\tepsilon\tth_cr\n")
		file.write("%3.2f %3.2f %3.2f %3.2f %3.2f\n" %(paramdict['radius'],paramdict['pos_bot_wall'],
...
def read_param(fname='../para_file.in'):
	## Membrane parameters
	N,coef_bending,Y,coef_vol_expansion = np.loadtxt(fname,skiprows=2,max_rows=1)
	N=int(N)
	radius,pos_bot_wall,sigma,epsilon,th_cr = np.loadtxt(fname,skiprows=4,max_rows=1)

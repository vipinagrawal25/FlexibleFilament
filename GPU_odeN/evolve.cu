...
struct EV TT;
struct EV *dev_tt;
void (*ALGO)( double [], double [], EV *, MPARAM *);
...
allowed-per-block then we launch in one way */
  ...
  while ( TT.time < TT.tmax){
    ...
    //euler<<<Nblock,Nthread>>>( dev_psi, dev_kk, dev_tt );
...
/*--------------------------- --------------------------------- */
__global__ void euler( double psi[], double k0[], EV *tt, MPARAM *dev_param){
  double dpsi[pp];
  ...
  if (tid < NN ){
    while( (*tt).time < (*tt).tdiag ){
      eval_rhs( dpsi, psi, tid, (*tt).time, dev_param );
      for (int ip=0; ip<pp; ip++){
        k0[ip+pp*tid]=dpsi[ip];
      }
      /*  I dont need to sync the threads here because each thread solves
...
/*-----------------------------------------------------------------------*/
__global__ void rnkt4( double psi[], double kk[], EV *tt, MPARAM *dev_param ){
  double dpsi[pp];
  ...
  // Make sure all the threads have done the sub-step. 
      __syncthreads( );
   // step 2 of rnkt4
      eval_rhs( dpsi, temp, tid, (*tt).time + (*tt).dt/2, dev_param );
      for (int ip=0; ip<pp; ip++){
  ...
  // Make sure all the threads have done the sub-step. 
      __syncthreads( );     
   // step 4 of rnkt4
      eval_rhs( dpsi, temp, tid, (*tt).time + (*tt).dt, dev_param );
      for (int ip=0; ip<pp; ip++){

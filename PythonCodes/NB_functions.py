...
def curv_compare(ax,dirname,omega,snaptime,is_ss=True,compute=False):
    nfigs = NP.shape(snaptime)[0]
    TT = 2*NP.pi/omega
    if compute:
        is_ss=False
    ...
    for ifig in range(nfigs):
        ...
        if(is_ss):
            ...
            ss_snap = NP.linspace(0,1.28,256)
        if compute:
            dd = NP.loadtxt(dirname+"/output/var"+str(isnap)+".txt")[:,0:2]
            kappa = compute_curv(dd)
        else:
            kappa=curv[isnap,1:]
        ax.plot(ss_snap,kappa,'.-',label =str(snaptime[ifig])+'T')
    ax.legend(fontsize=15)
...
def compute_curv(pos):
    NN = pos.shape[0]
    kappa = NP.zeros(NN)
    tangent=NP.diff(pos,axis=0)
    for iN in range(NN-1):
        tangent[iN,:]=tangent[iN,:]/NP.sqrt(tangent[iN,0]**2 + tangent[iN,1]**2)
    kappa[1:NN-1]=NP.cross(tangent[0:-1],tangent[1:])
    return kappa
...
def tracerPlot(ax,dirname,omega,cy,ntr=-1):
    ...
    if ntr==-1:
        ...
        ind = minTracerDisplacementIndex(dirname,omega,ntr,cy1=0,cy2=cy)

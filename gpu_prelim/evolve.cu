...
struct EV TT;
struct EV *dev_tt;
void (*ALGO)( double [], double [], EV *, MPARAM *, double *);
__global__ void euler( double psi[], double k0[], EV *tt, MPARAM *, double *);
__global__ void rnkt4( double psi[], double k0[], EV *tt, MPARAM *, double *);
...
allowed-per-block then we launch in one way */
  ...
  while ( TT.time < TT.tmax){
    ...
    ALGO<<<Nblock,Nthread>>>( dev_psi, dev_kk, dev_tt, dev_param , dev_diag);
...
__global__ void rnkt4( double psi[], double kk[], EV *tt,
  ...
  // Make sure all the threads have done the sub-step. 
      __syncthreads( );
   // step 2 of rnkt4
      eval_rhs( dpsi, temp, tid, (*tt).time + (*tt).dt/2, dev_param, diag );
      for (int ip=0; ip<pp; ip++){
  ...
  // Make sure all the threads have done the sub-step. 
      __syncthreads( );     
   // step 4 of rnkt4
      eval_rhs( dpsi, temp, tid, (*tt).time + (*tt).dt, dev_param, diag );
      for (int ip=0; ip<pp; ip++){
